...
export function MaterialTable({ materials, showLabor = false }: MaterialTableProps) {
      ...
      {Object.entries(grouped).map(([category, items]) => (
        <div key={category}>
          ...
          <div className="px-4 py-1.5 bg-white/2 border-b border-border text-[9px] tracking-[2px] uppercase font-bold text-muted">
            <Badge label={categoryLabels[category as MaterialCategory]} variant={category as MaterialCategory} size="sm" />
          </div>
          ...
          {items.map((material) => (
            ...
            >
              <div>
                <div className="text-sm font-medium text-light">{material.name}</div>
                {material.detail && (
                  <div className="text-[10px] text-muted mt-0.5">{material.detail}</div>
                )}
              </div>
