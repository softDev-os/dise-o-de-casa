...
export function MaterialTableSkeleton() {
  return (
    <div className="bg-card border border-border rounded-2xl overflow-hidden mb-6">
      ...
      {/* Rows */}
      {[1, 2, 3].map((i) => (
        <div key={i} className="grid grid-cols-[2fr_0.8fr_1fr_1fr] px-4 py-3 border-b border-border">
...
export function RoomCardSkeleton() {
  return (
    <div className="bg-card border border-border rounded-[14px] p-4 border-t-3 border-t-border">
      ...
      <Skeleton width={60} height={24} className="mt-3 pt-3 border-t border-border" />
    </div>
  );
}
