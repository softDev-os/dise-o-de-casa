...
export function usePDF() {
	...
	const generatePDF = async (quotation: Quotation): Promise<Blob> => {
		...
		try {
			...
			doc.setTextColor(100);
			doc.text(
				`Fecha: ${new Date(quotation.date).toLocaleDateString("es-CO")}`,
				20,
				40,
			);
			...
			const error =
				err instanceof Error ? err : new Error("Error generating PDF");
			setError(error);
			throw error;
		} finally {
	...
	const downloadPDF = async (quotation: Quotation, filename?: string) => {
		try {
			...
			link.download =
				filename ||
				`cotizacion-${quotation.title.toLowerCase().replace(/\s+/g, "-")}.pdf`;
