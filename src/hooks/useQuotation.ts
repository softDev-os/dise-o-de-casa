import { type Quotation, type Room, type Material, type PriceConfig } from '../types';
...
export function useQuotation() {
      ...
      const updatedMaterials = room.materials.map((mat) =>
        mat.id === materialId
          ? {
              ...mat,
              ...updates,
              total: (updates.quantity || mat.quantity) * (updates.unitPrice || mat.unitPrice),
  ...
  const recalculate = (quotation: Quotation): Quotation => {
    ...
    const laborTotal = updatedRooms.reduce((sum, room) => sum + room.laborTotal, 0);
    const unforeseen = calculateUnforeseen(subtotal, laborTotal, quotation.prices.imprevistos);
    const total = calculateTotal(subtotal, laborTotal, quotation.prices.imprevistos);
