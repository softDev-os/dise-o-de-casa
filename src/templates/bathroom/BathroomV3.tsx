...
export function BathroomV3() {
	...
	const paredMateriales = room.materials.filter(
		(m) => m.category === "revestimiento",
	);
	const techoMateriales = room.materials.filter(
		(m) => m.category === "techo" || m.category === "impermeabilizacion",
	);
	const accesorios = room.materials.filter(
		(m) => m.category === "accesorio" || m.category === "carpinteria",
	);
	...
	return (
		<div className="min-h-screen bg-bg">
			...
			<div className="bg-gradient-to-r from-gold/12 to-gold/5 border-l-3 border-gold px-5 py-3 text-xs text-gold flex items-center gap-2.5">
				⚠️ <strong>Medidas corregidas:</strong> El baño es 6 × 2.20 m (13.2 m² de
				piso). Paredes completas hasta ~2.40 m = 39.36 m². Las baldosas
				necesarias subieron de 48 a 322 unidades.
			</div>
						...
						<div>
							...
							<div className="bg-card border border-border rounded-2xl p-5 flex flex-col lg:flex-row gap-6 items-start mb-6">
								...
								<div className="flex-1 min-w-[200px] flex flex-col gap-2">
									...
									<div className="flex items-center gap-2 text-xs">
										...
										<span className="text-accent-2 font-semibold ml-auto">
											52.56 m²
										</span>
									</div>
									<div className="flex items-center gap-2 text-xs">
									...
									<div className="flex items-center gap-2 text-xs">
										...
										<span className="text-accent-2 font-semibold ml-auto text-base">
											322 und
										</span>
									</div>
								</div>
							...
							<div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
								<div className="bg-card border border-border rounded-xl p-4 border-t-[3px] border-t-accent">
									...
									<div className="font-display text-2xl text-light">
										13.2 <span className="text-xs text-muted">m²</span>
									</div>
								...
								<div className="bg-card border border-border rounded-xl p-4 border-t-[3px] border-t-gold">
									...
									<div className="font-display text-2xl text-light">
										39.36 <span className="text-xs text-muted">m²</span>
									</div>
								...
								<div className="bg-card border border-border rounded-xl p-4 border-t-[3px] border-t-blue">
									...
									<div className="font-display text-2xl text-light">
										322 <span className="text-xs text-muted">und</span>
									</div>
									<div className="text-[10px] text-muted">
										60×30 + 10% desp.
									</div>
						...
						<div>
							...
							<div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
								{accesorios.map((acc) => (
									<div
										key={acc.id}
										className="bg-card border border-border rounded-[14px] p-4 transition-all duration-200 hover:border-accent/30 hover:-translate-y-0.5"
									>
										...
										<div className="flex justify-between items-center pt-3 border-t border-border">
											<div className="text-[9px] tracking-[1px] uppercase text-muted">
												Precio
											</div>
